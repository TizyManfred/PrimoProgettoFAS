...
if [ -z "${VIRTUAL_ENV:-}" ]; then
    ...
    echo "[setup] Già all'interno di un virtual environment: $VIRTUAL_ENV"
fi

python3 -m pip install --upgrade pip --quiet --disable-pip-version-check
python3 -m pip install --quiet --disable-pip-version-check -r "$PROJECT_ROOT/python/requirements.txt"
